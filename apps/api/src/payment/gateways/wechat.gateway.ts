...
export class WechatGateway implements PaymentGateway {
  ...
  async refundTransaction(request: RefundRequest): Promise<RefundResult> {
    const endpoint = process.env.WECHAT_REFUND_API;
    if (!endpoint) {
      return { success: false, message: 'WECHAT_REFUND_API 未配置' };
    }
    try {
      const res = await fetch(endpoint, {
        ...
        headers: {
          ...
          ...(process.env.WECHAT_REFUND_TOKEN
            ? { Authorization: `Bearer ${process.env.WECHAT_REFUND_TOKEN}` }
            : {})
        },
        body: JSON.stringify({
      ...
      if (!res.ok) {
        this.logger.error(`WECHAT 退款接口失败 status=${res.status} body=${text.slice(0, 120)}`);
        return {
          success: false,
      ...
      const reason = error instanceof Error ? error.message : String(error);
      this.logger.error(`WECHAT 发起退款失败: ${reason}`);
