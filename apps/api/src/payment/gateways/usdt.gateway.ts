...
export class UsdtGateway implements PaymentGateway {
  readonly channel = PayChannel.USDT;
  private readonly logger = new Logger(UsdtGateway.name);
  ...
  async fetchTransactions(bizDate: string): Promise<RemoteTransaction[]> {
    const endpoint = process.env.USDT_RECONCILE_API;
    if (!endpoint) {
      this.logger.warn('USDT_RECONCILE_API 未配置，返回空账单');
      return [];
    }
    try {
      ...
      const res = await fetch(url.toString(), {
        headers: {
          ...
          ...(process.env.USDT_RECONCILE_TOKEN
            ? { Authorization: `Bearer ${process.env.USDT_RECONCILE_TOKEN}` }
            : {})
        }
      });
      if (!res.ok) {
        const body = await res.text();
        this.logger.error(`USDT 对账接口失败 status=${res.status} body=${body.slice(0, 120)}`);
        return [];
      }
      ...
      this.logger.error(`USDT 拉取对账失败: ${reason}`);
      return [];
    }
  }

  async refundTransaction(request: RefundRequest): Promise<RefundResult> {
    const endpoint = process.env.USDT_REFUND_API;
    if (!endpoint) {
      return { success: false, message: 'USDT_REFUND_API 未配置' };
    }
    try {
      const res = await fetch(endpoint, {
        ...
        headers: {
          ...
          ...(process.env.USDT_REFUND_TOKEN
            ? { Authorization: `Bearer ${process.env.USDT_REFUND_TOKEN}` }
            : {})
        },
        body: JSON.stringify({
      ...
      if (!res.ok) {
        this.logger.error(`USDT 退款接口失败 status=${res.status} body=${text.slice(0, 120)}`);
        return {
          success: false,
      ...
      const reason = error instanceof Error ? error.message : String(error);
      this.logger.error(`USDT 发起退款失败: ${reason}`);
